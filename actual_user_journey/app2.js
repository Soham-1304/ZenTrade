document.addEventListener('DOMContentLoaded', () => {
    ...
    let STATE = {};
    let niftyChart, singleStockChart, marketUpdateInterval, pulseUpdateInterval;
    ...
    const views = { 
        ...
        pulse: document.getElementById('pulse-view') // Added Pulse view
    };
    ...
    function runSimulationTick() {
        ...
        if (nifty) { const volatility = 0.00008; const change = (Math.random() - 0.5) * (nifty.price * volatility * 2); nifty.price = Math.max(0.01, nifty.price + change); nifty.change = nifty.price - nifty.day_open; nifty.changePct = nifty.day_open !== 0 ? (nifty.change / nifty.day_open) : 0; }
        const sensex = getStock('SENSEX');
        if (sensex) { const volatility = 0.00007; const change = (Math.random() - 0.5) * (sensex.price * volatility * 2); sensex.price = Math.max(0.01, sensex.price + change); sensex.change = sensex.price - sensex.day_open; sensex.changePct = sensex.day_open !== 0 ? (sensex.change / sensex.day_open) : 0; }
    }
    ...
    // --- NEW: PULSE FEATURE - DATA AGGREGATION ---
    function aggregateCommunityData() {
        ...
        allUsers.forEach(user => {
            if (user.accountData && user.accountData.orders) {
                user.accountData.orders.forEach(order => {
                    // Only consider trades from today
                    if (order.status === 'COMPLETE' && user.accountData.marketDate === todayString) {
                        if (order.type === 'BUY') {
                            buyVolume[order.instrument] = (buyVolume[order.instrument] || 0) + order.quantity;
                        } else {
                            sellVolume[order.instrument] = (sellVolume[order.instrument] || 0) + order.quantity;
                        }
                    }
                });
            }
    ...
    function updateWatchlist() {
        ...
        const filteredStocks = searchTerm === ''
            ? STOCKS_LIVE_DATA
            : STOCKS_LIVE_DATA.filter(stock => {
                const ticker = stock.name.toLowerCase();
                const stockName = stock.fullName ? stock.fullName.toLowerCase() : '';
                return ticker.includes(searchTerm) || stockName.includes(searchTerm);
            });

        // --- PULSE SENTIMENT INDICATOR ---
        const sentimentHtml = (stockName) => {
            if (STATE.pulse && STATE.pulse.sentiment && STATE.pulse.sentiment[stockName]) {
                const score = STATE.pulse.sentiment[stockName];
                if (score > 0.3) return `<span title="Community is Bullish" class="ml-2">🐂</span>`;
                if (score < -0.3) return `<span title="Community is Bearish" class="ml-2">🐻</span>`;
            }
            return '';
        };

        watchlistEl.innerHTML = filteredStocks.map(stock => {
            const priceClass = stock.change >= 0 ? 'text-positive' : 'text-negative';
            const isIndex = stock.exchange === 'INDEX';
            const actionButtons = isIndex ? '' : `<button data-action="buy" data-stock="${stock.name}" class="px-3 py-1 text-sm text-white bg-blue-500 hover:bg-blue-600 rounded-l">B</button><button data-action="sell" data-stock="${stock.name}" class="px-3 py-1 text-sm text-white bg-red-500 hover:bg-red-600">S</button>`;
            return `<div class="watchlist-item relative border-b border-color group hover-bg"><div class="flex justify-between items-center px-4 py-2"><div class="flex-1 overflow-hidden"><p class="font-semibold text-sm ${priceClass} flex items-center">${stock.name} ${sentimentHtml(stock.name)}</p><p class="text-xs text-secondary">${stock.exchange}</p></div><div class="flex items-center text-right"><p class="text-xs w-24"><span class="${priceClass}">${(stock.changePct || 0).toLocaleString('en-IN', { style: 'percent', minimumFractionDigits: 2 })}</span><span class="block text-primary">${(stock.change || 0).toFixed(2)}</span></p><p class="font-semibold text-sm w-20 text-primary">${stock.price.toFixed(2)}</p></div><div class="action-buttons absolute right-2 bg-secondary themed-shadow-lg rounded-md flex items-center border border-color">${actionButtons}<button data-action="view-chart" data-stock="${stock.name}" class="p-2 text-secondary hover-bg border-l border-color ${isIndex ? 'rounded-l' : ''}"><svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z"/></svg></button></div></div></div>`;
        }).join('');
    }
    ...
    function switchView(viewName) {
        ...
        // Always close dropdowns when switching views
        ...
        else if (viewName === 'pulse') PulseApp.renderPulseView(views.pulse);

        // Show the correct view and hide others
        Object.values(views).forEach(v => v.classList.add('hidden'));
        views[viewName].classList.remove('hidden');

        // Update all UI elements and navigation styling
        updateAllUI();
    ...
    function openFundsModal() { /* Function content from previous version remains the same */ if (!STATE.bankAccounts || STATE.bankAccounts.length === 0) { alert("No bank accounts linked. Please add one from your profile page first."); switchView('profile'); return; } let selectedAccountIndex = 0; const showAccountSelection = () => { const accountsHtml = STATE.bankAccounts.map((acc, index) => ` <div data-index="${index}" class="bank-account-item cursor-pointer flex justify-between items-center p-4 border rounded-lg ${index === selectedAccountIndex ? 'border-blue-500' : 'border-color'}"> <div> <p class="font-semibold text-primary">${acc.bankName}</p> <p class="text-sm text-secondary">A/C: ...${acc.accountNumber.slice(-4)}</p> </div> <p class="font-mono text-primary">${formatCurrency(acc.balance)}</p> </div> `).join(''); modalContainer.innerHTML = ` <div class="bg-secondary rounded-lg themed-shadow-lg p-6 w-full max-w-sm"> <div class="flex justify-between items-center mb-4"><h3 class="text-xl font-semibold text-primary">Add Funds</h3><button id="modal-close" class="text-secondary">×</button></div> <p class="text-sm text-secondary mb-4">Select an account to add funds from:</p> <div class="space-y-3 mb-6">${accountsHtml}</div> <div class="flex justify-end space-x-3"><button id="modal-cancel" class="px-4 py-2 bg-primary rounded hover-bg">Cancel</button><button id="modal-proceed" class="px-4 py-2 bg-blue-600 text-white rounded">Proceed</button></div> </div>`; document.querySelectorAll('.bank-account-item').forEach(item => { item.addEventListener('click', (e) => { selectedAccountIndex = parseInt(e.currentTarget.dataset.index, 10); showAccountSelection(); }); }); document.getElementById('modal-proceed').onclick = showAmountInput; document.getElementById('modal-close').onclick = closeModal; document.getElementById('modal-cancel').onclick = closeModal; }; const showAmountInput = () => { const selectedAccount = STATE.bankAccounts[selectedAccountIndex]; modalContainer.innerHTML = ` <div class="bg-secondary rounded-lg themed-shadow-lg p-6 w-full max-w-sm"> <div class="flex justify-between items-center mb-4"><h3 class="text-xl font-semibold text-primary">Add Funds</h3><button id="modal-close" class="text-secondary">×</button></div> <div class="mb-4"> <p class="text-sm text-secondary">From: ${selectedAccount.bankName} (...${selectedAccount.accountNumber.slice(-4)})</p> <p class="text-sm text-secondary">Available Balance: <span class="font-bold text-primary">${formatCurrency(selectedAccount.balance)}</span></p> </div> <div class="mb-4"> <label class="block text-sm font-medium text-secondary">Amount to Add (INR)</label> <input type="number" id="modal-amount" class="mt-1 block w-full p-2 border border-color bg-primary rounded" placeholder="Enter amount" min="100"> </div> <div class="flex justify-end space-x-3"><button id="modal-back" class="px-4 py-2 bg-primary rounded hover-bg">Back</button><button id="modal-confirm" class="px-4 py-2 bg-blue-600 text-white rounded">Confirm</button></div> </div>`; document.getElementById('modal-confirm').onclick = () => { const amountInput = document.getElementById('modal-amount'); const amount = parseFloat(amountInput.value); if (isNaN(amount) || amount <= 0) { alert('Please enter a valid amount.'); return; } if (amount > selectedAccount.balance) { alert('Insufficient funds in the selected bank account.'); return; } STATE.funds.equity += amount; selectedAccount.balance -= amount; saveState(); closeModal(); updateAllUI(); }; document.getElementById('modal-back').onclick = showAccountSelection; document.getElementById('modal-close').onclick = closeModal; }; showAccountSelection(); modalOverlay.style.display = 'block'; modalContainer.style.display = 'block'; }
    ...
    // --- UI UPDATE FUNCTION ---
    function updateAllUI() { if (STATE.currentView === 'dashboard') updateDashboardData(); else if (STATE.currentView === 'orders') updateOrdersData(); else if (STATE.currentView === 'holdings') updateHoldingsData(); updateHeader(); updateWatchlist(); updateNiftyChart(); }
    ...
    async function init() {
        ...
        if (!STATE.pulse) STATE.pulse = { mostBought: [], mostSold: [], sentiment: {} }; // Initialize Pulse state
        ...
        PulseApp.initialize(STATE); // Initialize Pulse module
        ...
        if (isNewDay) {
            ...
            catch (error) { STOCKS_MASTER_LIST = getFallbackData(); }
        ...
        document.getElementById('logout-button').addEventListener('click', () => { clearInterval(marketUpdateInterval); clearInterval(pulseUpdateInterval); sessionStorage.removeItem(SESSION_KEY); window.location.href = 'login.html'; });
        ...
        document.addEventListener('click', (e) => {
            ...
            if(viewTarget && profileDropdown && profileDropdown.contains(viewTarget)) {
        ...
        pulseUpdateInterval = setInterval(aggregateCommunityData, 15000); // Pulse data updates every 15 seconds
        await marketUpdateLoop();
        aggregateCommunityData(); // Run once on startup
    }
